...
map<string, vector<string>> computeAdjacentWords(const vector<string> &words) {
  ...
  for (auto &entry : wordsByLength) {
    ...
    for (int i = 0; i < groupNum; ++i) {
      ...
      // Words with same representatives are adjacent; so first populate a
      // map...
...
// character with the most words. Return a list of these words, in case of a
// tie.
vector<string>
...
// each word pair
vector<string> findChain(const vector<string> &words, const string &first,
                         const string &second) {
